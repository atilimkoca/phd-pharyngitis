...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        self.convertor = nn.Linear(56, num_features)

        # Linear layer
        #self.classifier = nn.Linear(num_features, num_classes)
        self.classifier = nn.Linear(13030400, num_classes)
        # Official init from torch repo.
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal_(m.weight.data)
            elif isinstance(m, SynchronizedBatchNorm2d):
                print('-----------------------------------in SynchronizedBatchNorm2d')
    ...
    def forward(self, x):
        ...
        features =  self.convertor(x1)
        out = F.relu(features, inplace=True)
        #out = F.avg_pool2d(out, kernel_size=7, stride=1).view(features.size(0), -1)
        #print(f'features - out= {out.shape}')
        out = F.avg_pool2d(out,kernel_size=7, stride=1).view(features.size(0), -1)
...
class iAFF(nn.Module):
    def __init__(self,in_c,r):
        ...
        self.ms_cam1 = ms_cam(in_c,r)
        self.ms_cam2 = ms_cam(in_c, r)
...
if __name__ == '__main__':
    ...
    print(model.features[5:7])
    y = model._forward(x)
    for i in range(len(y)):
        print(y[i].size())
    # y = model.features[:](x)
    # print(y.shape)
    # x2 = torch.rand([2, 64, 6, 6])
    #
    # model = ms_fusion(32,64,4)
    # y = model(x,x2)
    # print(y.size())
